...
async def check_user_authorization(client, user_id):
   """Check if user is member of any authorized groups"""
...
async def check_user(message):
    ...
    is_authorized = await check_user_authorization(message._client, message.from_user.id)
    if not is_authorized:
        await message.reply(Messages.NOT_AUTHORIZED)
        await message.stop_propagation() 
        return
